...
export default class SalveAnalyzer extends EventEmitter {
	constructor({mode = null, scramble}) {
		...
		this.state = {
			mode: 'cfop', // 'cfop' | 'roux' | null (unknown)
			cross: null,
			cll: null,
			pll: null,
			oll: null,
		...
		return config.stagesData[this.state.mode || 'cfop'].map(({id}, index, stagesData) => {
			...
			if (!this.state.cross) {
				return {
					...
					inspectionTime: stage.inspectionTime,
					moves: stage.sequence.length
				};
			}
	...
	toSolveEntry({time, date, isError}) {
		...
		return {
			...
			_lseoTime: lseoTime,
			_lsepTime: lsepTime,
			_f2l1ITime: f2l1ITime,
			_f2l2ITime: f2l2ITime,
			_f2l3ITime: f2l3ITime,
			_f2l4ITime: f2l4ITime,
			_ollITime: ollITime,
			_pllITime: pllITime,
			_f2l1ETime:f2l1ETime,
			_f2l2ETime:f2l2ETime,
			_f2l3ETime:f2l3ETime,
			_f2l4ETime:f2l4ETime,
			_ollETime: ollETime,
			_pllETime: pllETime,
			_crossMoves: crossMoves,
			_f2l1Moves: f2l1Moves,
			_f2l2Moves: f2l2Moves,
			_f2l3Moves: f2l3Moves,
			_f2l4Moves: f2l4Moves,
			_ollMoves: ollMoves,
			_pllMoves: pllMoves,
		};
	}
