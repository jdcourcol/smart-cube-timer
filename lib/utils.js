...
export const findCross = (cube) => {
	for (const face of faces) {
		...
		if (isCrossed && face === 'U') {// I always go go white cross
			return face;
		}
	}
	return null;
...
export const updateStages = ({stages, mode, stagesData, cross, time}) => {
	stagesData[mode].map(({id, name, color, dark, showInspection}, index, array_stages) =>{
		const stage = stages[id];
		const previousStage = index === 0 ? null : stages[array_stages[index - 1].id];
		const previousTime = previousStage ? previousStage.time : null;
		const deltaTime = (stage.time || time) - (previousTime === null ? 0 : previousTime);
		...
		const downFace = cross;

		const {inspection, execution} = (stages[id] && stage.sequence.length) ? getInspectionTime({stage, cross: downFace, previousTime}): {inspection: null, execution:null};
		if (execution > 0){
